    ...
    auto make_vm_description = [this, server, request, name, checked_args]() mutable -> VirtualMachineDescription {
        try
        {
            ...
            auto cloud_init_iso_path = config->factory->make_cloud_init_image(
                name, mp::utils::base_dir(vm_image.image_path), meta_data_cloud_init_config,
                user_data_cloud_init_config, vendor_data_cloud_init_config, network_data_cloud_init_config);

            VirtualMachineDescription vm_desc{request->num_cores() < std::stoi(mp::min_cpu_cores)
